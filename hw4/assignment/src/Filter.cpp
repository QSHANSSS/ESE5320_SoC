...
#define OUTPUT_WIDTH_EXTEN (INPUT_WIDTH - (FILTER_LENGTH - 1))+6
#define OUTPUT_WIDTH_MOD (OUTPUT_WIDTH - 2)

//unsigned Coefficients[] = {2, 15, 62, 98, 62, 15, 2};
unsigned char Coefficients[] = {2, 15, 62, 98, 62, 15, 2};

void Filter_horizontal(const unsigned char *Input, unsigned char *Output)
{
  for (int Y = 0; Y < INPUT_HEIGHT; Y++)
    for (int X = 0; X < OUTPUT_WIDTH_EXTEN; X++)
    {
...
void Filter_vertical_modified(const unsigned char *Input, unsigned char *Output)
{
    	uint16x8_t Coef0 = vdupq_n_u16(Coefficients[0]);
   	uint16x8_t Coef1 = vdupq_n_u16(Coefficients[1]);
   	uint16x8_t Coef2 = vdupq_n_u16(Coefficients[2]);
	...
	for (int X = 0; X < OUTPUT_WIDTH; X += 16)
  	{
	    ...
	    {
      		uint8x16_t Data6 = vld1q_u8(Input + (Y + 6) * OUTPUT_WIDTH + X);
	      	uint16x8_t Sum0H = vaddl_high_u8(Data0, Data6);
		   	uint16x8_t Sum1H = vaddl_high_u8(Data1, Data5);
		   	uint16x8_t Sum2H = vaddl_high_u8(Data2, Data4);
		    uint16x8_t SumH= vmulq_u16(Sum0H, Coef0);
		    uint16x8_t Sum=vmulq_u16(Sum1H, Coef1);
		  	SumH=vaddq_u16(Sum,SumH);
		    uint16x8_t SumK=vmulq_u16(Sum2H, Coef2);
		    Sum= vmulq_u16(vmovl_u8(vget_high_u8(Data3)),  Coef3);
		  	SumK=vaddq_u16(Sum,SumK);
			SumH=vaddq_u16(SumK,SumH);
		    SumH= vshrq_n_u16(SumH, 8);
			...
			uint16x8_t Sum0L = vaddl_u8(vget_low_u8(Data0), vget_low_u8(Data6));
		 	uint16x8_t Sum1L = vaddl_u8(vget_low_u8(Data1), vget_low_u8(Data5));
		   	uint16x8_t Sum2L = vaddl_u8(vget_low_u8(Data2), vget_low_u8(Data4));
		 	uint16x8_t SumL = vmulq_u16(Sum0L,  Coef0);
			Sum=vmulq_u16(Sum1L, Coef1);
			SumL=vaddq_u16(Sum,SumL);
		    uint16x8_t SumM = vmulq_u16(Sum2L, Coef2);   //set more reg so that computing multiplication in parallell
		    Sum= vmulq_u16(vmovl_u8(vget_low_u8(Data3)), Coef3);
			SumM=vaddq_u16(Sum,SumM);
			SumL=vaddq_u16(SumM, SumL);
		    SumL= vshrq_n_u16(SumL, 8);
...
void Filter_vertical_modified_optimized(const unsigned char *Input, unsigned char *Output)
{
    uint16x8_t Coef0 = vdupq_n_u16(Coefficients[0]);
   	uint16x8_t Coef1 = vdupq_n_u16(Coefficients[1]);
   	uint16x8_t Coef2 = vdupq_n_u16(Coefficients[2]);
	...
	for (int X = 0; X < OUTPUT_WIDTH; X += 16)
  	{
		...
		for (int Y = 0; Y < OUTPUT_HEIGHT; Y+=2)
			...
			Sum=vmulq_u16(Sum1L, Coef1);
			SumL=vaddq_u16(Sum,SumL);
		    uint16x8_t SumM = vmulq_u16(Sum2L, Coef2);
		    Sum= vmulq_u16(vmovl_u8(vget_low_u8(Data3)), Coef3);
			SumM=vaddq_u16(Sum,SumM);
			SumL=vaddq_u16(SumM, SumL);
		    SumL= vshrq_n_u16(SumL, 8);
			...
			Data6 = vld1q_u8(Input + (Y + 7) * OUTPUT_WIDTH + X);
	      	Sum0H = vaddl_high_u8(Data0, Data6);
		   	Sum1H = vaddl_high_u8(Data1, Data5);
		   	Sum2H = vaddl_high_u8(Data2, Data4);
		    SumH= vmulq_u16(Sum0H, Coef0);
		    Sum=vmulq_u16(Sum1H, Coef1);
		  	SumH=vaddq_u16(Sum,SumH);
		    SumK=vmulq_u16(Sum2H, Coef2);
		    Sum= vmulq_u16(vmovl_u8(vget_high_u8(Data3)),  Coef3);
		  	SumK=vaddq_u16(Sum,SumK);
			SumH=vaddq_u16(SumK,SumH);
		    SumH= vshrq_n_u16(SumH, 8);
			...
			ResultH = vmovn_u16(SumH);
	 	    ResultL = vmovn_u16(SumL);
		   	vst1q_u8(Output + (Y+1) * OUTPUT_WIDTH + X,  vcombine_u8(ResultL,ResultH));
