...
#(parameter
    ...
    output wire [63:0]                   buff,
    output wire [63:0]                   input_length,
    output wire [63:0]                   output_compress,
...
localparam
    ...
    ADDR_IER                    = 6'h08,
    ADDR_ISR                    = 6'h0c,
    ADDR_BUFF_DATA_0            = 6'h10,
    ADDR_BUFF_DATA_1            = 6'h14,
    ADDR_BUFF_CTRL              = 6'h18,
    ADDR_INPUT_LENGTH_DATA_0    = 6'h1c,
    ADDR_INPUT_LENGTH_DATA_1    = 6'h20,
    ADDR_INPUT_LENGTH_CTRL      = 6'h24,
    ADDR_OUTPUT_COMPRESS_DATA_0 = 6'h28,
    ADDR_OUTPUT_COMPRESS_DATA_1 = 6'h2c,
    ADDR_OUTPUT_COMPRESS_CTRL   = 6'h30,
...
//------------------------Local signal-------------------
    ...
    reg  [63:0]                   int_buff = 'b0;
    reg  [63:0]                   int_input_length = 'b0;
    reg  [63:0]                   int_output_compress = 'b0;
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_BUFF_DATA_0: begin
                    rdata <= int_buff[31:0];
                end
                ADDR_BUFF_DATA_1: begin
                    rdata <= int_buff[63:32];
                end
                ...
                ADDR_OUTPUT_COMPRESS_DATA_0: begin
                    rdata <= int_output_compress[31:0];
                end
                ADDR_OUTPUT_COMPRESS_DATA_1: begin
                    rdata <= int_output_compress[63:32];
                end
...
assign buff            = int_buff;
assign input_length    = int_input_length;
assign output_compress = int_output_compress;
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_BUFF_DATA_0)
            int_buff[31:0] <= (WDATA[31:0] & wmask) | (int_buff[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_BUFF_DATA_1)
            int_buff[63:32] <= (WDATA[31:0] & wmask) | (int_buff[63:32] & ~wmask);
    end
end
...
// int_output_compress[31:0]
always @(posedge ACLK) begin
    if (ARESET)
        int_output_compress[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_OUTPUT_COMPRESS_DATA_0)
            int_output_compress[31:0] <= (WDATA[31:0] & wmask) | (int_output_compress[31:0] & ~wmask);
    end
end

// int_output_compress[63:32]
always @(posedge ACLK) begin
    if (ARESET)
        int_output_compress[63:32] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_OUTPUT_COMPRESS_DATA_1)
            int_output_compress[63:32] <= (WDATA[31:0] & wmask) | (int_output_compress[63:32] & ~wmask);
    end
end
