...
//{
    ...
    //for(int i=0;i<sizeof(message);i++)
      //  printf("%c",message[i]);
    /*char shaSum[SHA3_384_DIGEST_SIZE+3];
    wc_Sha3 sha3_384; padding
...
//    Sha sha;
//    wc_InitSha(&sha);
//    wc_ShaUpdate(&sha, (const unsigned char*)message, strlen((const char*)message));
...
//    }
//    printf("\n");
//    int judge=match(chunk_hash,shaSum,index);
//    return judge;
//}
...
int sha(unsigned char *buff, uint32_t length /*char **chunk_hashunordered_map<string,int> table*/){
	
	int rem=length%64;
	int empty_spot = 64-rem;
	int final_length = length + empty_spot;
	if(length/64<=8){
		final_length += 64;
	}
	uint8_t data[final_length]={0};
	memcpy(data,buff,length);
	data[length] = 128;
    data[final_length-1]=length;
    data[final_length-2]=length>>8;
    data[final_length-3]=length>>16;
    data[final_length-4]=length>>24;
	uint32_t state[8];
	sha256_process_arm(state, data, length);
    	char shaSum[8];
    	shaSum[0]=(uint8_t)(state[0] >> 24);
        shaSum[1]=(uint8_t)(state[0] >> 16);
        shaSum[2]=(uint8_t)(state[0] >>  8);
        shaSum[3]=(uint8_t)(state[0] >>  0);
    	shaSum[4]=(uint8_t)(state[1] >> 24);
        shaSum[5]=(uint8_t)(state[1] >> 16);
        shaSum[6]=(uint8_t)(state[1] >>  8);
        shaSum[7]=(uint8_t)(state[1] >>  0);
    	//int judge=match(chunk_hash,shaSum,index);
        int judge=match_map(/*table,*/shaSum);
       /* printf("hash:");
        for(int i=0;i<2;i++)
            printf("%d",state[i]);
        printf("\n");*/
