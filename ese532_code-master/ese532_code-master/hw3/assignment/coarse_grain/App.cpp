...
int main()
{
  ...
  for (int Frame = 0; Frame < FRAMES; Frame++)
  {
    ...
    ths.push_back(std::thread(&Filter_horizontal, Input_data + Frame * FRAME_SIZE, Temp_data[0], 0, INPUT_HEIGHT_SCALE / 2));
    ths.push_back(std::thread(&Filter_vertical, Input_data + Frame * FRAME_SIZE, Temp_data[0], 0, INPUT_WIDTH_SCALE / 2));
    ths.push_back(std::thread(&Filter_horizontal, Input_data + Frame * FRAME_SIZE, Temp_data[0], INPUT_HEIGHT_SCALE / 2, INPUT_HEIGHT_SCALE));
    ths.push_back(std::thread(&Filter_vertical, Input_data + Frame * FRAME_SIZE, Temp_data[0], INPUT_WIDTH_SCALE / 2, INPUT_WIDTH_SCALE));
    ths.push_back(std::thread(&Differentiate, Input_data + Frame * FRAME_SIZE, Temp_data[0], 0, INPUT_HEIGHT_SCALE /2));
    ths.push_back(std::thread(&Differentiate, Input_data + Frame * FRAME_SIZE, Temp_data[0], INPUT_HEIGHT_SCALE / 2, INPUT_HEIGHT_SCALE));
