...
void Filter_vertical_new(const unsigned char *Input, unsigned char *Output)
{
  // modify the following code
  uint8x8_t Coef0 = vdupq_n_u8(Coefficients[0]);
  uint8x8_t Coef1 = vdupq_n_u8(Coefficients[1]);
  uint8x8_t Coef2 = vdupq_n_u8(Coefficients[2]);
  uint8x8_t Coef3 = vdupq_n_u8(Coefficients[3]);
  for (int X = 0; X < OUTPUT_WIDTH; X += 16)
  {
    uint8x16_t Data0 = vld1q_u8(Input + 0 * OUTPUT_WIDTH + X);
   	uint8x16_t Data1 = vld1q_u8(Input + 1 * OUTPUT_WIDTH + X);
    ...
    {
   	  uint8x16_t Data6 = vld1q_u8(Input + (Y + 6) * OUTPUT_WIDTH + X);
   	  uint16x8_t SumH = vmulq_u16(vmovl_u8(vget_high_u8(Data0), Coef0);
   	  SumH = vmlaq_u16(SumH, Data0, Coef1);
   	  SumH = vmlaq_u16(SumH, Sum2H, Coef2);
   	  SumH = vmlaq_u16(SumH, vmovl_u8(vget_high_u8(Data6)), Coef3);
   	  SumH = vshrq_n_u16(SumH, 8);
   	  uint8x8_t ResultH = vmovn_u16(SumH);
   	  uint16x8_t SumL = vmulq_u16(Sum0L, Coef0);
   	  SumL = vmlaq_u16(SumL, Sum1L, Coef1);
   	  ...
   	  Sum1 = vshrq_n_u16(SumL, 8);
      Sum2= vshlq_n_u16(Sum1, 8);
   	  uint8x8_t ResultL = vmovn_u16(Sum1);
      uint8x8_t ResultH = vmovn_u16(Sum2);
   	  vst1q_u8(Output + Y * OUTPUT_WIDTH + X, vcombine_u8(ResultL, ResultH));
   	  Data0 = Data1;
   	  Data1 = Data2;
   	  Data2 = Data3;
...
{
  for (int Y = 0; Y < OUTPUT_HEIGHT; Y++)
    for (int X = 0; X < OUTPUT_WIDTH+6; X++)
    {
      unsigned int Sum = 0;
      for (int i = 0; i < FILTER_LENGTH; i++)
