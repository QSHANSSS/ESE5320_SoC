...
# 2 "<built-in>" 2
# 1 "fpga/hls/MMult.cpp" 2
# 1 "fpga/hls/MMult.h" 1
...
# 6 "fpga/hls/MMult.h" 2
# 23 "fpga/hls/MMult.h"
extern "C" {
void mmult_fpga(float A[130 * 32 * 32], float B[130 * 32 * 32],
                float C[130 * 32 * 32]);
}
void mmult_fpga_optimize(float A[130 * 32 * 32], float B[130 * 32 * 32],
      float C[130 * 32 * 32]);
...
void pass_dataflow(float *input1,
     ...
     float *output,
     int intnumInputs);
# 2 "fpga/hls/MMult.cpp" 2
# 1 "/tools/Xilinx/Vitis_HLS/2020.2/common/technology/autopilot/ap_int.h" 1
# 3 "fpga/hls/MMult.cpp" 2
# 41 "fpga/hls/MMult.cpp"
...
#pragma HLS PIPELINE
 inStream1.write(input1[c * 32 * 32 + i * 32 + j]);

              }
 }
 VITIS_LOOP_53_3: for (int i = 0; i < 32; i++) {
          VITIS_LOOP_54_4: for (int j = 0; j < 32; j++) {
#pragma HLS PIPELINE

 inStream2.write(input2[c * 32 * 32 + i * 32 + j]);
           }
 }
}
void write(hls::stream<float> &outStream,
    ...
    int intnumInputs, int c)
{
       VITIS_LOOP_65_1: for (int i = 0; i < 32; i++) {
         VITIS_LOOP_66_2: for (int j = 0; j < 32; j++) {
#pragma HLS PIPELINE
 output[c * 32 * 32 + i * 32 + j]=outStream.read();
...
#pragma HLS array_partition variable=in2 block factor=16 dim=1
 VITIS_LOOP_83_1: for (int i = 0; i < 32; i++)
              {
                VITIS_LOOP_85_2: for (int j = 0; j < 32; j++)
                {
#pragma HLS PIPELINE
 in1[i][j] = inStream1.read();
              ...
              VITIS_LOOP_93_3: for (int i = 0; i < 32; i++)
              {

                VITIS_LOOP_96_4: for (int j = 0; j < 32; j++)
                {
#pragma HLS PIPELINE
 float result = 0;
                  VITIS_LOOP_100_5: for (int k = 0; k < 32; k++)
                  {
...
#pragma HLS DATAFLOW
 ...
 hls::stream<float > inStream1;
   hls::stream<float > inStream2;
   hls::stream<float > outStream;
  VITIS_LOOP_120_1: for (int c = 0; c < 130; c++)
  {
  read(input1, input2, inStream1, inStream2, intnumInputs,c);
  exec(inStream1,inStream2, outStream, intnumInputs,c);
  write(outStream, output, intnumInputs, c);
  }
}
extern "C" {
void pass(float *input1,
  float *input2,
   float *output,
   unsigned int numInputs,
   int c) {
#pragma HLS INTERFACE m_axi port=input1 offset=slave bundle=p0
#pragma HLS INTERFACE m_axi port=input2 offset=slave bundle=p1
#pragma HLS INTERFACE m_axi port=output offset=slave bundle=p2

 pass_dataflow(input1, input2,output,numInputs);
}
}
void mmult_fpga(float A[130 * 32 * 32], float B[130 * 32 * 32],
      float C[130 * 32 * 32])
